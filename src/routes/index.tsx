...
import { Bike, Flame, Leaf, MapPin, ShoppingBag, Star, Timer, Wallet } from "lucide-react";
...
import { menuItemsQueryOptions } from "@/lib/queries";
...
export const Route = createFileRoute("/")({
  ...
  loader: async ({ context }) => {
    await context.queryClient.ensureQueryData(menuItemsQueryOptions());
  },
  component: HomePage,
});
    ...
    <>
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-10 px-4 py-14 sm:px-6 sm:py-20 lg:grid-cols-2 lg:py-24">
          <div className="fade-up">
            ...
            <p className="mt-5 max-w-lg text-base leading-relaxed text-muted-foreground sm:text-lg">
              From local Ghanaian favourites to burgers, pizza and Chinese-inspired dishes, there's
            ...
            <div className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-muted-foreground">
              <span className="flex items-center gap-1.5">
                <Star className="h-4 w-4 fill-accent text-accent" /> 4.8 from 600+ Kasoa orders
              </span>
              <span className="flex items-center gap-1.5">
          ...
          <div className="relative">
            ...
            <div className="absolute -bottom-5 left-5 hidden rounded-2xl bg-card px-5 py-4 shadow-lift sm:block">
              <p className="font-display text-sm font-semibold">Ready in 25 minutes</p>
              <p className="text-xs text-muted-foreground">Pickup at Kasoa New Market</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-bold sm:text-4xl">Featured dishes</h2>
          </div>
        ...
        <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
          {CATEGORY_TILES.map((tile) => (
            ...
            >
              ...
              />
              <div className="p-4">
                <h3 className="font-display text-base font-semibold">{tile.name}</h3>
                <p className="mt-1 text-xs text-muted-foreground">{tile.blurb}</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="mt-16 bg-secondary/60 py-16">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          <h2 className="text-3xl font-bold sm:text-4xl">Why choose KIN Kitchen</h2>
          <p className="mt-2 max-w-xl text-muted-foreground">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <div className="rounded-3xl border border-border/70 bg-card p-8 shadow-soft">
            ...
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              We deliver across Kasoa — Ofaakor, Opeikuma, Millennium City, Toll Booth, Bortianor
              Junction and nearby areas. Flat GH₵ 15 fee, typical arrival in 45 minutes. Rider calls
              you on arrival.
            </p>
          </div>
          <div className="rounded-3xl border border-border/70 bg-card p-8 shadow-soft">
            ...
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              Collect free of charge from our kitchen opposite Kasoa New Market. Most pickup orders
              are boxed and ready in about 25 minutes, daily from 9:00am to 10:00pm.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="gradient-charcoal relative overflow-hidden rounded-[2rem] px-6 py-14 text-center text-primary-foreground sm:px-12">
          <h2 className="text-3xl font-bold sm:text-4xl">Hungry? Let's get cooking.</h2>
          <p className="mx-auto mt-3 max-w-xl text-primary-foreground/70">
            Build your order in under two minutes, pay with cash or Mobile Money, and we'll handle
            the rest.
          </p>
